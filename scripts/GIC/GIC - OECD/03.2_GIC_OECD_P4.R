...
gic<-ggplot(gic_df, aes(x = percentile * 100, y = growth_rate)) +
  ...
  annotate("text", x = 95, y = mean_growth + 0.002, label = "Mean Growth", color = "black", size = 3, hjust = 1) +
  annotate("text", x = 95, y = median_growth + 0.00002, label = "Median Growth", color = "black", size = 3, hjust = 1)

print(gic)

ggsave(
  filename = "output/OECD Graphs/GIC_P4.png",  
  plot     = gic,                        
  width    = 8,                        
  height   = 5,                           
  dpi      = 300                           
