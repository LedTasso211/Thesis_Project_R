...
gic<-ggplot(gic_df, aes(x = percentile * 100, y = growth_rate)) +
  ...
  labs(
    ...
    title = "Growth Incidence Curve for Georgia (2004-2007)"
  ) +
  ...
  annotate("text", x = 95, y = mean_growth + 0.001, label = "Growth in Mean", color = "black", size = 3, hjust = 1) +
  annotate("text", x = 95, y = median_growth + 0.001, label = "Median", color = "black", size = 3, hjust = 1)

print(gic)

ggsave(
  filename = "output/Graphs/GIC_P1.png",  
  plot     = gic,                        
  width    = 8,                            
  height   = 5,                           
  dpi      = 300                         
...
p_values <- c(0.1, 0.15, 0.2, 0.25, 0.4, 1)

mean_growths <- sapply(p_values, function(p) {
  mean(gic_df$growth_rate[gic_df$percentile <= p]) * 100  
})

growth_rates_df<-data.frame(
  p = p_values * 100, 
  mean_growth_rate = round(mean_growths, 1)
)

save_results_to_excel(growth_rates_df, sheet_name = "P1", output_file = "output/Tables/Growth_Rates.xlsx")