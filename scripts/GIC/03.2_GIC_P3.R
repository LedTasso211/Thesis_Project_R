...
gic<-ggplot(gic_df, aes(x = percentile * 100, y = growth_rate)) +
  ...
  annotate("text", x = 95, y = mean_growth + 0.0004, label = "Growth in Mean", color = "black", size = 3, hjust = 1) +
  annotate("text", x = 95, y = median_growth + 0.000001, label = "Median", color = "black", size = 3, hjust = 1)

print(gic)

ggsave(
  filename = "output/Graphs/GIC_P3.png",  
  plot     = gic,                        
  width    = 8,                            
  height   = 5,                           
  dpi      = 300                           
...
save_results_to_excel(growth_rates_df, sheet_name = "P3", output_file = "output/Tables/Growth_Rates.xlsx")